...
%   7. Interpolated Pointwise modulus (depth-modulus in 'nm' and 'kPa')
%   8. Fitted apparent modulus (kPa)
%
% AZELOGLU E.U. (c) 2016
% Modification by Robert J. Wiener, 2018
...
% clear;
if nargin < 2
    [selectedFiles,path_name] =  uigetfile('*.csv', 'Multiselect', 'on');
else
    
end

for runs = 1:length(selectedFiles')
    file_name = selectedFiles(runs);
...
shift_test_mode = 2; % Make this a variable
...
for c_n = 1:no_of_curves
    ...
    % ---------------------------------------------------------------------
    % Start the post-contract fitting process from 50% of the maximum
