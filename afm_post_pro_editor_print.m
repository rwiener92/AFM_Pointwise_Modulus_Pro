function [curves,total_error] = afm_post_pro_editor(path_name,file_name,...
    geometry_opt,pointwise_opt)
% function [curves,total_error] = afm_post_pro(geometry_opt,pointwise_opt)
%   Produces a CURVES matrix that is composed of 8 key components:
...
%   6. Pointwise Moduli curve (depth-modulus in 'nm' and 'kPa')
%   7. Interpolated Pointwise moduli (depth-modulus in 'nm' and 'kPa')
%   8. Fitted apparent modulus (kPa)
%   9. If the contact point fitted curve was 'Deleted' or 'Edited'
%
% AZELOGLU E.U. (c) 2016
% Modification By:  Robert J. Wiener 2016
...
% Record of curve modifications
curve_mod_log = 'Original';
% During user check of contact point guestdlg options 'Yes' 'No' 'Edit'
% -------------------------------------------------------------------------
...
% location.
%
% Option to linearly interpret pointwse moduli data:
liniterp_opt = 1;
% If lininterp_opt = 0, code ignores function for linear interpolation for depth
% dependent moduli
% If lininterp_opt = 1, will call function E_DEPTH_LININTERP for linear
% interpolation of previously calculated depthwise moduli at specified
% depth intervals
%
% -------------------------------------------------------------------------
...
while (check_opt == 2) || (check_opt == -1)
    ...
    % Ginput edit option for user defined contact point
    % Edit portion only applies to ext data i.e. extension_opt == 1
    elseif check_opt == -1
            pre_final = extension(1:hand_selected_index ,:);
            post_final = extension(hand_selected_index+1:end ,:);
        end
        ...
        if liniterp_opt == 1;
        disp('Interpolating pointwise moduli data to symetric depth intervals...')
            % Secondary function which linearly interpolates E_pointwise data
            % for symetic interval comparison accross multiple data sets
            % Min/Max bounds for desired pointwise data at speficied depth intervals
            min_depth = 20;
            max_depth = 400;
            stp_depth = 10; % nanometers
            
            [E_INT_pointwise] = ...
                E_DEPTH_LININTERP(E_pointwise,min_depth,max_depth,stp_depth);
        ...
        if shift_test_mode == 0      
            ...
            subplot(2,3,1),plot(extension(:,1),extension(:,2),... % Extend
                'k', retract(:,1),retract(:,2),'k');            % Retract
            title('Raw AFM Deflection Curve');
            set(gca,'XGrid','on','YGrid','on');
            desired_ax = axis;

            if extension_opt == 1
                subplot(2,3,2),plot(pre_final(:,1),pre_final(:,2),'.g',...
                    post_final(:,1),post_final(:,2),'.m');
            elseif extension_opt == 2
                subplot(2,3,2),plot(pre_final(:,1),pre_final(:,3),'.g',...
            ...
            subplot(2,3,4),plot(FC(:,1),FC(:,2),'.m');
            title('Post Contact Force vs. Indentation Depth');
            set(gca,'XGrid','on','YGrid','on');

            subplot(2,3,5),plot(E_pointwise(:,1),E_pointwise(:,2),'r');
        ...
        % User will zoom to visually inspect contact location
        % strike ENETER to continue program
        % dialog box will ask for program-contact point accuracy
        % User may now opt for redefined contact point
        zoom on
        disp('PRESS ENTER AFTER ZOOM')
        pause
        zoom off
        check_button = questdlg('Do you want to accept force data?',...
            'Edit Contact Point','Yes','No','Edit','Yes');
        if strcmp(check_button,'Yes')
            check_opt = 0; % keep contact point
        elseif strcmp(check_button,'No')
            check_opt = 1; % trash this force curve
            E_pointwise(:,:) = NaN;
            E_apparent = NaN;
            E_INT_pointwise(:,:) = NaN;
            curve_mod_log = 'Deleted'; 
        elseif strcmp(check_button,'Edit')
            check_opt = -1; % redefine contact point
            curve_mod_log = 'Edit';
        end
        clear check_button
        
        % define graphical user input as new contact point
        % repeate loop with new use defined contact point
        % writen for extension curve
        % ginput(1): exact selection on graph, may not be exact data point
        % min function adjusts user ginput(1) to nearest x-data point
           if check_opt == -1
           [hand_selected_point,~] = ginput(1); 
           [hand_selected_point, hand_selected_index] = ...
                          min(abs(extension(:,1) - hand_selected_point));
           close
           end
end    
        ...
        curves{c_n,7} = E_INT_pointwise;
        curves{c_n,8} = E_apparent;
        curves{c_n,9} = curve_mod_log;
        
        % Keeps storage variable for all interpolated pointwise data
        E_INT_storage(:,c_n) = E_INT_pointwise(:,2);
        E_APP_storage(c_n) = curves{c_n,8};
    end
    
    clear total_err E_apparent E_pointwise FC E_INT_pointwise
...
if c_n == no_of_curves
    ...
    % Calculates overall average of cells apparent modulus
    curves{c_n+1,8} = nanmean(E_APP_storage);
    curves{no_of_curves+1,2} = 'AVERAGE' ;
    
    % Creates temp matrix for average modlui at specified depth intervals
    S = size(E_INT_storage);
    for n_INT = 1:S(1)
    E_temp(n_INT,1) = nanmean(E_INT_storage(n_INT,:));
    end
    % Sets {curve} slot to average pointwise modulus array for entire cell
    curves{c_n+1,7} = E_temp;
    
end


end
%stringIND = strfind(file_name,'.csv');
%assignin('base', file_name(1:stringIND-1), curves);